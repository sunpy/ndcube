...
class Length1Tabular(_Tabular):
    ...
    def __init__(self, points=None, lookup_table=None, point_width=None, value_width=None,
                 method="linear", bounds_error=True, fill_value=np.nan, **kwargs) -> None:
        ...
        if len(lookup_table) != 1:
            msg = "lookup_table must have length 1."
    ...
    def evaluate(self, x):
        ...
        idx = diff == margin if margin.value == 0 else np.logical_and(diff >= -1 * margin, diff < margin)
...
def _generate_tabular(lookup_table, interpolation="linear", points_unit=u.pix, **kwargs):
    ...
    if not isinstance(lookup_table, u.Quantity):
        msg = "lookup_table must be a Quantity."
...
class BaseTableCoordinate(abc.ABC):
    ...
    def __init__(self, *tables, mesh=False, names=None, physical_types=None) -> None:
        self.table = tables
        self.mesh = mesh
...
class QuantityTableCoordinate(BaseTableCoordinate):
    ...
    def __init__(self, *tables, names=None, physical_types=None) -> None:
        if not all(isinstance(t, u.Quantity) for t in tables):
            msg = "All tables must be astropy Quantity objects"
            raise TypeError(msg)
        if not all(t.unit.is_equivalent(tables[0].unit) for t in tables):
            msg = "All tables must have equivalent units."
    ...
    def __getitem__(self, item):
        ...
        if not (len(item) == len(self.table) or len(item) == self.table[0].ndim):
            msg = "Can not slice with incorrect length"
            raise ValueError(msg)
    ...
    def interpolate(self, *new_array_grids, **kwargs):
        ...
        """
        if self.is_scalar():
            msg = "Cannot interpolate a scalar QuantityTableCoordinate."
            raise ValueError(msg)
        # Sanitize input.
        ndim = self.ndim
        if len(new_array_grids) != ndim:
            msg = f"A new array grid must be given for each array axis/table, i.e. {ndim}"
            raise ValueError(
                msg)
        if any(new_grid.shape != new_array_grids[0].shape for new_grid in new_array_grids):
            msg = "New array grids must all be same shape."
            raise ValueError(msg)
...
class SkyCoordTableCoordinate(BaseTableCoordinate):
    ...
    def __init__(self, *tables, mesh=False, names=None, physical_types=None) -> None:
        if len(tables) != 1 and isinstance(tables[0], SkyCoord):
            msg = "SkyCoordLookupTable can only be constructed from a single SkyCoord object"
            raise ValueError(msg)
        if mesh and tables[0].ndim > 1:
            msg = "If mesh is True, input SkyCoord must be 1-D."
            raise ValueError(msg)
        ...
        if names is not None and len(names) != n_components:
            msg = (
                "The number of names must equal number of components in the input "
                             f"SkyCoord: {n_components}."
            )
            raise ValueError(msg)
        if physical_types is not None and len(physical_types) != n_components:
            msg = (
                "The number of physical types must equal number of components in "
    ...
    def combine_slices(slice1, slice2):
        ...
        if all(ints):
            msg = "Can not combine two integers"
            raise ValueError(msg)
        if any(ints):
    ...
    def interpolate(self, *new_array_grids, mesh_output=None, **kwargs):
        ...
        """
        if self.is_scalar():
            msg = "Cannot interpolate a scalar SkyCoordTableCoordinate."
            raise ValueError(msg)
        ...
        shape = self.shape
        if len(new_array_grids) != ndim:
            msg = f"A new array grid must be given for each array axis, i.e. {ndim}"
            raise ValueError(msg)
        if any(new_grid.shape != new_array_grids[0].shape for new_grid in new_array_grids):
            msg = "New array grids must all be same shape."
            raise ValueError(msg)
        if mesh_output is None:
            mesh_output = False if new_array_grids[0].ndim > 1 else self.mesh
...
class TimeTableCoordinate(BaseTableCoordinate):
    ...
    def __init__(self, *tables, names=None, physical_types=None, reference_time=None) -> None:
        if len(tables) != 1 and isinstance(tables[0], Time):
            msg = "TimeLookupTable can only be constructed from a single Time object."
            raise ValueError(msg)
        ...
        if names is not None and len(names) != 1:
            msg = "A Time coordinate can only have one name."
            raise ValueError(msg)
        if physical_types is not None and len(physical_types) != 1:
            msg = "A Time coordinate can only have one physical type."
            raise ValueError(msg)
    ...
    def interpolate(self, new_array_grids, **kwargs):
        ...
        """
        if self.is_scalar():
            msg = "Cannot interpolate a scalar TimeTableCoordinate."
            raise ValueError(msg)
...
class MultipleTableCoordinate(BaseTableCoordinate):
    ...
    def __init__(self, *table_coordinates) -> None:
        if not all(isinstance(lt, BaseTableCoordinate) and
                   not (isinstance(lt, MultipleTableCoordinate)) for lt in table_coordinates):
            msg = (
                "All arguments must be BaseTableCoordinate instances, such as QuantityTableCoordinate, "
    ...
    def __str__(self) -> str:
        ...
        joiner = ",\n " + (len(classname) + 8) * " " if length > np.get_printoptions()["linewidth"] else ", "
    ...
    def __and__(self, other):
        ...
        others = other._table_coords if isinstance(other, MultipleTableCoordinate) else [other]
    ...
    def __getitem__(self, item):
        ...
        if len(item) != self.n_inputs:
            msg = f"length of the slice ({len(item)}) must match the number of coordinates {self.n_inputs}"
            raise ValueError(
                msg)
