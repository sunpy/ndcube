    ...
    def __init__(self, seq, plot_axis_index=-1, axis_ranges=None, unit_x_axis=None,
        ...
        if axis_ranges is None:
            ...
            if plot_axis_index == 0:
                ...
                # Get x-axis values from each cube and combine into a
                # single array for axis_ranges kwargs.
                x_axis_coords = []
                for i, cube in enumerate(seq.data):
                    # Get the x-axis coordinates for each cube.
                    #x_axis_cube_coords = cube.axis_world_coords(cube_plot_axis_index).to(unit_x_axis).value
                    x_axis_cube_coords = cube.axis_world_coords(cube_plot_axis_index).value
                    ...
                    if x_axis_cube_coords.shape != cube.data.shape:
                        ...
                        dependent_axes = utils.wcs.get_dependent_data_axes(
                            cube.wcs, cube_plot_axis_index, cube.missing_axis)
            ...
            if xlabel is None:
                xlabel = "{0} [{1}]".format(seq.world_axis_physical_types[plot_axis_index], unit_x_axis)
            if ylabel is None:
