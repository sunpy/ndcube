...
class ReorderedLowLevelWCS(BaseWCSWrapper):
    ...
    def __init__(self, wcs, pixel_order, world_order) -> None:
        if sorted(pixel_order) != list(range(wcs.pixel_n_dim)):
            msg = f"pixel_order should be a permutation of {list(range(wcs.pixel_n_dim))}"
            raise ValueError(msg)
        if sorted(world_order) != list(range(wcs.world_n_dim)):
            msg = f"world_order should be a permutation of {list(range(wcs.world_n_dim))}"
            raise ValueError(msg)
        self._wcs = wcs
