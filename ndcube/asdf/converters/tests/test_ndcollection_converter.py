...
@pytest.mark.skipif(
    Version(gwcs_version) < Version("0.20"), reason="Requires gwcs>=0.20"
)
...
@pytest.fixture
def create_ndcollection_sequence(
    ndcube_gwcs_3d_ln_lt_l, ndcube_gwcs_3d_ln_lt_l_ec_dropped_dim
):

    sequence02 = NDCubeSequence(
        [ndcube_gwcs_3d_ln_lt_l, ndcube_gwcs_3d_ln_lt_l_ec_dropped_dim]
    )
    sequence20 = NDCubeSequence(
        [ndcube_gwcs_3d_ln_lt_l_ec_dropped_dim, ndcube_gwcs_3d_ln_lt_l]
    )
    return NDCollection(
        [("seq0", sequence02), ("seq1", sequence20)], aligned_axes="all"
    )
