...
class QuantityTableCoordinateConverter(Converter):
    ...
    def from_yaml_tree(self, node, tag, ctx):
        ...
        mesh = node.get("mesh")
        physical_types = node.get("physical_types")
        quantitytablecoordinate = QuantityTableCoordinate(
            *node["table"], names=names, physical_types=physical_types
        )
...
class SkyCoordTableCoordinateConverter(Converter):
    ...
    def from_yaml_tree(self, node, tag, ctx):
        ...
        mesh = node.get("mesh")
        physical_types = node.get("physical_types")
        return SkyCoordTableCoordinate(
            node["table"], mesh=mesh, names=names, physical_types=physical_types
        )
