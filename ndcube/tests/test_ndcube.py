...
@pytest.mark.parametrize(
    ...
    [(cubem[:, 1],
     ...
     (cube[:, 0:2], # 8th
      NDCube,
      mask_cube[:, 0:2],
      _wcs_slicer(wt, [False, False, False, True], (slice(None, None, None), slice(0, 2, None))),
      uncertainty[:, 0:2],
      u.Quantity((2, 2, 4), unit=u.pix),
     ...
     (cube[0, 1],
      NDCube,
      mask_cube[1, 1],
      _wcs_slicer(wt, [False, False, False, True], (0, 1)),
      uncertainty[1, 1],
      u.Quantity((4, ), unit=u.pix),
      tuple(['time']),
     ...
     (cube[1, 0:2],
      NDCube,
      mask_cube[1, 0:2],
      _wcs_slicer(wt, [False, False, False, True], (1, slice(0, 2, None))),
      uncertainty[1, 0:2],
      u.Quantity((2, 4), unit=u.pix),
      ('em.wl', 'time'),
      {'time': {'axis': None, 'value': u.Quantity(1, unit=u.pix)},
     ...
     (cube[1, :],
      NDCube,
      mask_cube[1, :],
      _wcs_slicer(wt, [False, False, False, True], (1, slice(None, None, None))),
      uncertainty[1, :],
      u.Quantity((3, 4), unit=u.pix),
      ('em.wl', 'time'),
      {'bye': {'axis': 1, 'value': u.Quantity([0., 1., 2., 3.], unit=u.pix)},
       'custom:pos.helioprojective.lat': {'axis': None, 'value': u.Quantity(0.99974625, unit=u.deg)},
       'hello': {'axis': 0, 'value': u.Quantity([0., 1., 2.], unit=u.pix)},
       'time': {'axis': None, 'value': u.Quantity( 1., unit=u.pix)}}
      )])
...
@pytest.mark.parametrize(
    ...
    [(cubem[1],
     ...
     (cube[:],
      NDCube,
      mask_cube[:],
      _wcs_slicer(wt, [False, False, False, True], slice(None, None, None)),
      uncertainty[:],
      u.Quantity((2, 3, 4), unit=u.pix),
      ('custom:pos.helioprojective.lat', 'em.wl', 'time'),
       None
...
@pytest.mark.parametrize(
    ...
    [(cubem[:, :, 1],  #1
     ...
     (cube[1, 1, :],  #24
      NDCube,
      mask_cube[1, 1, :],
      _wcs_slicer(wt, [False, False, False, True], (1, 1, slice(None, None, None))),
      uncertainty[1, 1, :],
      u.Quantity((4, ), unit=u.pix),
      tuple(['time']),
      {'time': {'axis': None, 'value': u.Quantity(1, unit=u.pix)},
