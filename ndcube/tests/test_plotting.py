...
@pytest.mark.parametrize("test_input, test_kwargs, expected_values", [
    (cube[0, 0], {},
     (list, matplotlib.lines.Line2D, u.Quantity([0.4, 0.8, 1.2, 1.6], unit="min"),
      np.array([1, 2, 3, 4]), "", "", (0.4, 1.6), (1, 4)))
    ])
def test_cube_plot_1D(test_input, test_kwargs, expected_values):
    # Unpack expected properties.
    expected_type1, expected_type2, expected_xdata, expected_ydata, \
