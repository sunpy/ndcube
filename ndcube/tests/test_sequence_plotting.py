...
data_cube2 = np.zeros((5, 2, 3, 4))
data_cube2 = np.array([data2 for _ in range(5)])
...
cube1 = NDCube(
    data_cube, wt,
    extra_coords=[
        ('pix', 0, u.Quantity(range(data_cube.shape[0]), unit=u.pix)),
        ('hi', 1, u.Quantity(range(data_cube.shape[1]), unit=u.s)),
...
cube1_with_mask = NDCube(
    data_cube, wt,
    mask=np.zeros_like(data_cube, dtype=bool),
    extra_coords=[
        ('pix', 0, u.Quantity(range(data_cube.shape[0]), unit=u.pix)),
        ('hi', 1, u.Quantity(range(data_cube.shape[1]), unit=u.s)),
        ('distance', None, u.Quantity(0, unit=u.cm)),
...
cube1_with_unit_and_uncertainty = NDCube(
    data_cube, wt,
    unit=u.km, uncertainty=np.sqrt(data_cube),
    extra_coords=[
        ('pix', 0, u.Quantity(range(data_cube.shape[0]), unit=u.pix)),
        ('hi', 1, u.Quantity(range(data_cube.shape[1]), unit=u.s)),
        ('distance', None, u.Quantity(0, unit=u.cm)),
...
cube3 = NDCube(
    data_cube2, wt,
    extra_coords=[
        ('pix', 0, u.Quantity(np.arange(1, data_cube2.shape[0]+1), unit=u.pix) +
         cube1.extra_coords['pix']['value'][-1]),
        ('hi', 1, u.Quantity(range(data_cube2.shape[1]), unit=u.s)),
...
cube3_with_mask = NDCube(
    data_cube2, wt,
    mask=np.zeros_like(data_cube2, dtype=bool),
    extra_coords=[
        ('pix', 0, u.Quantity(np.arange(1, data_cube2.shape[0]+1), unit=u.pix) +
         cube1.extra_coords['pix']['value'][-1]),
        ('hi', 1, u.Quantity(range(data_cube2.shape[1]), unit=u.s)),
...
cube3_with_uncertainty = NDCube(
    data_cube2, wt,
    uncertainty=np.sqrt(data_cube2),
    extra_coords=[
        ('pix', 0, u.Quantity(np.arange(1, data_cube2.shape[0]+1), unit=u.pix) +
         cube1.extra_coords['pix']['value'][-1]),
        ('hi', 1, u.Quantity(range(data_cube2.shape[1]), unit=u.s)),
...
cube3_with_unit_and_uncertainty = NDCube(
    data_cube2, wt,
    unit=u.m, uncertainty=np.sqrt(data_cube2),
    extra_coords=[
        ('pix', 0, u.Quantity(np.arange(1, data_cube2.shape[0]+1), unit=u.pix) +
         cube1.extra_coords['pix']['value'][-1]),
        ('hi', 1, u.Quantity(range(data_cube2.shape[1]), unit=u.s)),
...
x_axis_coords3 = np.array([[2.0e-11, 4.0e-11, 6.0e-11, 2.0e-11],
          [4.0e-11, 4.5e-11, 3.5e-11, 4.0e-11],
          [6.0e-11, 2.0e-11, 4.0e-11, 6.0e-11]]).reshape((1, 1, 1, 3, 4))
new_x_axis_coords3_shape = u.Quantity(seq.dimensions, unit=u.pix).value.astype(int)
new_x_axis_coords3_shape[-2] = 1
new_x_axis_coords3_shape[-1] = 1
none_axis_ranges_axis3 = [np.arange(0, len(seq.data)),
                          np.array([0., 1., 2., 3., 4.]), np.arange(0, 2),
                          np.tile(np.array(x_axis_coords3), new_x_axis_coords3_shape), np.arange(4)]
...
distance0_none_axis_ranges_axis0 = [edges_to_centers_nd(_get_extra_coord_edges(seq.sequence_axis_extra_coords["distance"].value),0),
                                    np.array([0, 1, 2, 3, 4]), np.arange(0, 2),
                                    np.arange(0, int(seq.dimensions[-2].value)), np.arange(4) ]
...
hi2_none_axis_ranges_axis2 = [
    np.arange(0, len(seq.data)), np.array([0, 1, 2, 3, 4]),
    np.array([0, 1]), np.arange(0, int(seq.dimensions[-2].value)), np.arange(4)]

x_axis_coords1 = np.zeros(tuple([int(s.value) for s in seq.dimensions]))
x_axis_coords1[0, 0] = x_axis_coords1[2, 0] = 0
x_axis_coords1[0, 1] = x_axis_coords1[2, 1] = 1.
x_axis_coords1[0, 2] = x_axis_coords1[2, 2] = 2.
x_axis_coords1[0, 3] = x_axis_coords1[2, 3] = 3.
x_axis_coords1[0, 4] = x_axis_coords1[2, 4] = 4.


x_axis_coords1[1, 0] = x_axis_coords1[3, 0] =  5
x_axis_coords1[1, 1] = x_axis_coords1[3, 1] =  6.
x_axis_coords1[1, 2] = x_axis_coords1[3, 2] =  7.
x_axis_coords1[1, 3] = x_axis_coords1[3, 3] =  8.
x_axis_coords1[1, 4] = x_axis_coords1[3, 4] =  9.


pix1_none_axis_ranges_axis1 = [
    np.arange(0, len(seq.data)), x_axis_coords1, np.arange(0, 2),
    np.arange(0, int(seq.dimensions[-2].value)), np.arange(4)]

# Derive expected extents
seq_axis1_lim_deg = [0.49987220943356364, 0.4999927890804696]
...
cubelike_none_axis_ranges_axis0 = [[-0.5, 19.5], np.arange(0, 2),
                                   np.arange(0, int(seq.cube_like_dimensions[-2].value)), np.arange(4)]


# @pytest.mark.parametrize("test_input, test_kwargs, expected_values", [
#     (seq[:, 0, 0, 0, 0], {},
#      (np.arange(len(seq.data)), np.array([1, 11,  1, 11]),
#       "meta.obs.sequence [None]", "Data [None]", (0, len(seq[:, 0, 0, 0, 0].data)-1),
#       (min([cube.data.min() for cube in seq[:, 0, 0, 0, 0].data]),
#        max([cube.data.max() for cube in seq[:, 0, 0, 0, 0].data])))),

#     (seq_with_units[:, 0, 0, 0, 0], {},
#      (np.arange(len(seq_with_units.data)), np.array([1, 0.011,  1, 0.011]),
#       "meta.obs.sequence [None]", "Data [km]", (0, len(seq_with_units[:, 0, 0, 0, 0].data)-1),
#       (min([(cube.data * cube.unit).to(seq_with_units[:, 0, 0, 0, 0].data[0].unit).value
#             for cube in seq_with_units[:, 0, 0, 0, 0].data]),
#        max([(cube.data * cube.unit).to(seq_with_units[:, 0, 0, 0, 0].data[0].unit).value
#             for cube in seq_with_units[:, 0, 0, 0, 0].data])))),

#     (seq_with_uncertainty[:, 0, 0, 0, 0], {},
#      (np.arange(len(seq_with_uncertainty.data)), np.array([1, 11,  1, 11]),
#       "meta.obs.sequence [None]", "Data [None]", (0, len(seq_with_uncertainty[:, 0, 0, 0, 0].data)-1),
#       (min([cube.data for cube in seq_with_uncertainty[:, 0, 0, 0, 0].data]),
#        max([cube.data for cube in seq_with_uncertainty[:, 0, 0, 0, 0].data])))),

#     (seq_with_units_and_uncertainty[:, 0, 0, 0, 0], {},
#      (np.arange(len(seq_with_units_and_uncertainty.data)), np.array([1, 0.011,  1, 0.011]),
#       "meta.obs.sequence [None]", "Data [km]",
#       (0, len(seq_with_units_and_uncertainty[:, 0, 0, 0, 0].data)-1),
#       (min([(cube.data*cube.unit).to(seq_with_units_and_uncertainty[:, 0, 0, 0, 0].data[0].unit).value
#             for cube in seq_with_units_and_uncertainty[:, 0, 0, 0, 0].data]),
#        max([(cube.data*cube.unit).to(seq_with_units_and_uncertainty[:, 0, 0, 0, 0].data[0].unit).value
#             for cube in seq_with_units_and_uncertainty[:, 0, 0, 0, 0].data])))),

#     (seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0], {},
#      (np.arange(len(seq_with_units_and_some_uncertainty.data)), np.array([1, 0.011,  1, 0.011]),
#       "meta.obs.sequence [None]", "Data [km]",
#       (0, len(seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0].data)-1),
#       (min([(cube.data*cube.unit).to(
#           seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0].data[0].unit).value
#           for cube in seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0].data]),
#        max([(cube.data*cube.unit).to(
#            seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0].data[0].unit).value
#            for cube in seq_with_units_and_some_uncertainty[:, 0, 0, 0, 0].data])))),

#     (seq[:, 0, 0, 0, 0], {"axes_coordinates": "distance"},
...
#     (seq[:, 0, 0, 0, 0], {"axes_coordinates": u.Quantity(np.arange(len(seq.data)), unit=u.cm),
#                        "axes_units": u.km},
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_values", [
    (seq[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512]),
      np.array([1,  1,  1,  1,  1, 11, 11, 11, 11, 11,  1,  1,  1,  1,  1, 11, 11,
       11, 11, 11]),
      "{0} [{1}]".format(seq[:, :, 0, 0, 0].cube_like_world_axis_physical_types[common_axis], "deg"),
    ...
    (seq_with_units[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       ...
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512]),
      np.array([1., 1., 1., 1., 1., 0.011, 0.011, 0.011, 0.011,
       0.011, 1., 1., 1., 1., 1., 0.011, 0.011, 0.011,
       0.011, 0.011]),
      "{0} [{1}]".format(seq[:, :, 0, 0, 0].cube_like_world_axis_physical_types[common_axis], "deg"),
    ...
    (seq_with_uncertainty[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
      ...
      np.array([1,  1,  1,  1,  1, 11, 11, 11, 11, 11,  1,  1,  1,  1,  1, 11, 11,
       11, 11, 11]),
      "{0} [{1}]".format(
          seq_with_uncertainty[:, :, 0, 0, 0].cube_like_world_axis_physical_types[
              common_axis], "deg"),
      "Data [None]", tuple(seq_axis1_lim_deg),
      (min([cube.data.min() for cube in seq_with_uncertainty[:, :, 0, 0, 0].data]),
       max([cube.data.max() for cube in seq_with_uncertainty[:, :, 0, 0, 0].data])))),

    (seq_with_some_uncertainty[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
       0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512]),
      np.array([ 1,  1,  1,  1,  1, 11, 11, 11, 11, 11,  1,  1,  1,  1,  1, 11, 11,
       11, 11, 11]),
      "{0} [{1}]".format(
          seq_with_some_uncertainty[:, :, 0, 0, 0].cube_like_world_axis_physical_types[
              common_axis], "deg"),
      "Data [None]", tuple(seq_axis1_lim_deg),
      (min([cube.data.min() for cube in seq_with_some_uncertainty[:, :, 0, 0, 0].data]),
       max([cube.data.max() for cube in seq_with_some_uncertainty[:, :, 0, 0, 0].data])))),
    ...
    (seq_with_units_and_uncertainty[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
      ...
      np.array([1., 1., 1., 1., 1., 0.011, 0.011, 0.011, 0.011,
       ...
       0.011, 0.011]),
      "{0} [{1}]".format(
          seq_with_units_and_uncertainty[:, :, 0, 0, 0].cube_like_world_axis_physical_types[
              common_axis], "deg"),
      "Data [km]", tuple(seq_axis1_lim_deg),
      (min([min((cube.data * cube.unit).to(u.km).value)
            for cube in seq_with_units[:, :, 0, 0, 0].data]),
    ...
    (seq_with_units_and_some_uncertainty[:, :, 0, 0, 0], {},
     (np.array([0.49987769, 0.49993858, 0.49997512, 0.49998731, 0.49997512,
      ...
      np.array([1., 1., 1., 1., 1., 0.011, 0.011, 0.011, 0.011,
       ...
       0.011, 0.011]),
      "{0} [{1}]".format(
          seq_with_units_and_some_uncertainty[:, :, 0, 0, 0].cube_like_world_axis_physical_types[
              common_axis], "deg"),
      "Data [km]", tuple(seq_axis1_lim_deg),
      (min([min((cube.data * cube.unit).to(u.km).value)
            for cube in seq_with_units[:, :, 0, 0, 0].data]),
    ...
    (seq[:, :, 0, 0, 0], {"axes_coordinates": "pix"},
     (seq[:, :, 0, 0, 0].common_axis_extra_coords["pix"].value,
      np.array([1,  1,  1,  1,  1, 11, 11, 11, 11, 11,  1,  1,  1,  1,  1, 11, 11,
       11, 11, 11]), "pix [pix]", "Data [None]",
      (min(seq[:, :, 0, 0, 0].common_axis_extra_coords["pix"].value),
       max(seq[:, :, 0, 0, 0].common_axis_extra_coords["pix"].value)),
      (min([cube.data.min() for cube in seq[:, :, 0, 0, 0].data]),
       max([cube.data.max() for cube in seq[:, :, 0, 0, 0].data])))),

    (seq[:, :, 0, 0, 0],
     {"axes_coordinates": np.arange(10, 10+seq[:, :, 0, 0, 0].cube_like_dimensions[0].value)},
     (np.arange(10, 10 + seq[:, :, 0, 0, 0].cube_like_dimensions[0].value),
      np.array([1,  1,  1,  1,  1, 11, 11, 11, 11, 11,  1,  1,  1,  1,  1, 11, 11,
       11, 11, 11]),
      "{0} [{1}]".format("", None), "Data [None]",
      (10, 10 + seq[:, :, 0, 0, 0].cube_like_dimensions[0].value - 1),
      (min([cube.data.min() for cube in seq[:, :, 0, 0, 0].data]),
       max([cube.data.max() for cube in seq[:, :, 0, 0, 0].data]))))
    ])
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_error", [
    (seq[:, :, 0, 0, 0], {"axes_coordinates": [
        np.arange(10, 10+seq[:, :, 0, 0, 0].dimensions[-1].value), None],
        "axes_units": [u.m, None]}, ValueError),

    (seq[:, :, 0, 0, 0], {"axes_coordinates": [
        None, np.arange(10, 10+seq[:, :, 0, 0, 0].dimensions[0].value)],
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_error", [
    (seq[:, :, :, 0, 0], {"axes_coordinates": [
        np.arange(10, 10+seq[:, :, :, 0, 0].cube_like_dimensions[-1].value), None],
        "axes_units": [u.m, None]}, ValueError),

    (seq[:, :, :, 0, 0], {"axes_coordinates": [
        None, np.arange(10, 10+seq[:, :, :, 0, 0].cube_like_dimensions[0].value)],
        "axes_units": [None, u.m]}, ValueError)
    ])
...
#     (seq, {}, seq_stack.reshape(4, 1, 2, 3, 4, 5)),
#     (seq_with_units, {}, seq_stack_km.reshape(4, 1, 2, 3, 4, 5))
#     ])
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_data", [
    (seq, {}, seq_concat.reshape(20, 2, 3, 4)),
    (seq_with_units, {}, seq_concat_km.reshape(20, 2, 3, 4))
    ])
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_values", [
    (seq, {"plot_axis_indices": 3},
     (seq_stack.data, none_axis_ranges_axis3, "em.wl [m]", "Data [None]",
      (2.0000000000000002e-11, 6e-11),
      (seq_stack.data.min(), seq_stack.data.max()))),

    (seq_with_units, {"plot_axis_indices": -2, "data_unit": u.km},
     (seq_stack_km.data, none_axis_ranges_axis3, "em.wl [m]", "Data [km]",
      (2.0000000000000002e-11, 6e-11),
...
@pytest.mark.parametrize("test_input, test_kwargs, expected_values", [
    # (seq, {"plot_axis_indices": 3, "axes_units": u.s},
    ...
    (seq, {"plot_axis_indices": 0},
     (seq_concat.data, cubelike_none_axis_ranges_axis0,
      ...
      (0, 19), (seq_concat.data.min(), seq_concat.data.max()))),
    ...
    (seq_with_some_masks, {"plot_axis_indices": -4},
     (seq_concat.data, cubelike_none_axis_ranges_axis0,
      "custom:pos.helioprojective.lon [deg]", "Data [None]",
      (0, 19), (seq_concat.data.min(), seq_concat.data.max()))),
